...
non_spenders<-player_df %>%
  ...
  select(max_day,active_days,max_session,gametime_hrs,furthest_level,coin_spent,
         ...
         median_time_taken,has_decreasing_session_lengths,avg_session_length_mins,median_time_taken_post_level_30,game_time_mins_to_reach_level_10,
         game_time_mins_to_reach_level_25,game_time_mins_to_reach_level_60,gold_balance_depleted_level)
...
#make an initial SOM to revert to in case of error
som_grid1 <- somgrid(xdim = xd, ydim=yd, topo="hexagonal",toroidal=T)

xd<-next_iteration_params[1]$xdim
yd<-next_iteration_params[1]$ydim
rlen<-next_iteration_params[1]$rl
alpha1<- next_iteration_params[1]$a1
alpha2<- next_iteration_params[1]$a2
radius<-next_iteration_params[1]$r


#generate SOM with the input parameters
first_som          <-     som(non_spenders, 
                            grid=som_grid1, 
...
####### 
   ...
   # clamp_parameters(xd,yd,radius,rlen,default_som_length,max_map_aspect_ratio,min_radius_recip,max_radius_recip,min_rlen,max_rlen)
    ...
    #generate SOM with the input parameters
    som_grid <- somgrid(xdim = xd, ydim=yd, topo="hexagonal",toroidal=T)
    non_spenders.som <- tryCatch({som(non_spenders, 
                                  grid=som_grid, 
                                  rlen=rlen, #number of iterations
                                  alpha=c(alpha1,alpha2), #learning rate
                                  radius=radius, # default is 2/3 of map length
                                  keep.data = TRUE)
    }, error = function(e) {return(first_som)})
...
xdim<-elites[1,]$xdim
ydim<-elites[1,]$ydim
rlen<-100000#elites[1,]$rl
alpha1<-elites[1,]$a1
alpha2<-elites[1,]$a2
radius<-elites[1,]$r


som_grid <- somgrid(xdim = xdim, ydim=ydim, topo="hexagonal",toroidal=T)
