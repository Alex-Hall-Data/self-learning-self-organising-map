...
data1<-player_df%>%
  ...
  select(max_day,active_days,max_session,gametime_hrs,furthest_level,revenue,num_iap_purchases,coin_spent,
...
non_spenders.som <- som(non_spenders, 
                 grid=som_grid, 
                 rlen=100000, #number of iterations
                 alpha=c(0.0001,0.00001), #learning rate
                 keep.data = TRUE)
...
#plot clusters
som_cluster <- cutree(hclust(dist(non_spenders.som$codes[[1]])), 13)
